...
public final class Shapes {
    ...
    /** Filled triangle ABC, grown by {@code rounding} (at most 8 px) for soft corners. */
    public static void triangle(GuiGraphics graphics, float ax, float ay, float bx, float by, float cx, float cy,
                                float rounding, int color) {
        float pad = rounding + MARGIN;
        float x0 = Math.min(ax, Math.min(bx, cx)) - pad;
        float y0 = Math.min(ay, Math.min(by, cy)) - pad;
        float x1 = Math.max(ax, Math.max(bx, cx)) + pad;
        float y1 = Math.max(ay, Math.max(by, cy)) + pad;
        submit(graphics, x0, y0, x1, y1,
                x0 - ax, y0 - ay, x1 - ax, y1 - ay,
                fixed(bx - ax), fixed(by - ay), fixed(cx - ax), fixed(cy - ay),
                0.25f, Math.max(0f, Math.min(8f, rounding)), color);
    }
    ...
    private static void submit(GuiGraphics graphics, float x0, float y0, float x1, float y1,
                               ...
                               int a, int b, int c, int d, float mode, float rounding, int color) {
        ...
        graphics.guiRenderState.submitGuiElement(new ShapeRenderState(pose, x0, y0, x1, y1, lx0, ly0, lx1, ly1,
                a, b, c, d, mode, rounding, color, scissor, bounds));
    }
