...
| `killcam` | повтор последних 10 секунд перед смертью; «Мои реплеи» — сохранённые убийства, смерти и клипы (клавиша «Сохранить момент», J) |
...
- `config/skirmish/replays/*.skreplay` — сохранённые реплеи KillCam: заголовок JSON + сжатые кадры (формат версионирован, битые и чужие файлы пропускаются). Лимиты по числу и размеру; самые старые без звёздочки удаляются первыми.
